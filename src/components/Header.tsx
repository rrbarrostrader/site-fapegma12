...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-primary/95 backdrop-blur-sm border-b border-navy-light/30">
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        <a href="#inicio" className="flex items-center gap-2">
          ...
          <div className="text-primary-foreground">
            ...
            <span className="block text-xs text-primary-foreground/70 -mt-1">Conjunto Vitória</span>
          </div>
        </a>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-primary border-t border-navy-light/30 px-4 pb-4">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
          ...
          <Button variant="whatsapp" size="sm" className="mt-2 w-full" asChild>
            <a href="https://wa.me/5500000000000" target="_blank" rel="noopener noreferrer">
              <Phone className="w-4 h-4" />
              WhatsApp
            </a>
